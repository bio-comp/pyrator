...
def _get_bin_edges(
    ...
    if bin_type == "quantile":
        ...
        return np.percentile(data_clean, percentiles)  # type: ignore[no-any-return]
    elif bin_type == "fd":
        ...
        return np.linspace(data_clean.min(), data_clean.max(), n_bins_calc + 1)
    elif bin_type == "scott":
        sigma = data_clean.std()
        bin_width = 3.5 * sigma * (len(data_clean) ** (-1 / 3))
        bin_width = max(bin_width, 1e-10)
        data_range = data_clean.max() - data_clean.min()
        n_bins_calc = max(1, int(np.ceil(data_range / bin_width)))
        return np.linspace(data_clean.min(), data_clean.max(), n_bins_calc + 1)
    elif bin_type == "rice":
        n_bins_calc = max(1, int(np.ceil(2 * (len(data_clean) ** (1 / 3)))))
        return np.linspace(data_clean.min(), data_clean.max(), n_bins_calc + 1)
    elif bin_type == "sturges":
        n_bins_calc = max(1, int(np.ceil(np.log2(len(data_clean)) + 1)))
        return np.linspace(data_clean.min(), data_clean.max(), n_bins_calc + 1)
    elif bin_type == "sqrt":
        n_bins_calc = max(1, int(np.ceil(np.sqrt(len(data_clean)))))
        return np.linspace(data_clean.min(), data_clean.max(), n_bins_calc + 1)
    else:
...
def _bin_data(
    ...
    return pd.cut(data, bins=bin_edges.tolist(), labels=False, include_lowest=True)


def _calculate_psi(
    baseline_data: pd.Series,
    current_data: pd.Series,
    bins: Literal["quantile", "fd", "scott", "rice", "sturges", "sqrt"],
    n_bins: int,
    cutpoints: list[float] | None,
    eps: float,
) -> float:
    """Calculate PSI between baseline and current series."""
    is_numeric = pd.api.types.is_numeric_dtype(baseline_data)

    if is_numeric:
        bin_edges = _get_bin_edges(baseline_data, bin_type=bins, n_bins=n_bins, cutpoints=cutpoints)
    else:
    ...
    n_bins_actual = int(max_bin) + 2

    baseline_counts = np.bincount(baseline_binned.dropna().astype(int), minlength=n_bins_actual)
    current_counts = np.bincount(current_binned.dropna().astype(int), minlength=n_bins_actual)
    ...
    for current_window in current_windows:
        ...
        if stratify:
            ...
            for _, stratum_row in baseline_strata.iterrows():
                ...
                psi_value = _calculate_psi(
                    stratum_baseline, stratum_current, bins, n_bins, cutpoints, eps
                )
            ...
            psi_value = _calculate_psi(baseline_data, current_data, bins, n_bins, cutpoints, eps)
